...
EXP_NAME = "so101_mujoco_reach_joint_baseline"
ALGORITHM = "scripted_joint_reach_baseline"
CONTROL_TYPE = None
...
def get_candidate() -> dict[str, Any]:
    return {
        "description": (
            "SO-101 MuJoCo reach seed with a lightweight deterministic joint-position "
            "baseline. It provides a runnable benchmark/control surface for the new "
            "mesh-free SO-101 environment; learning-oriented policies should mutate "
            "this file into SAC/PPO or IK-assisted control."
        ),
        "recipe": {
            "algorithm": ALGORITHM,
...
class Agent:
    ...
    def act(self, obs: Any, deterministic: bool = False) -> np.ndarray:
        ...
        if self.action_dim >= 5:
            action[0] = np.clip(action[0] + 1.8 * delta[1], -1.0, 1.0)
            action[1] = np.clip(action[1] - 1.6 * delta[2] - 0.6 * delta[0], -1.0, 1.0)
            action[2] = np.clip(action[2] + 1.4 * delta[0] + 1.2 * delta[2], -1.0, 1.0)
            action[3] = np.clip(action[3] - 1.0 * delta[2], -1.0, 1.0)
        if self.action_dim >= 6:
            action[5] = self.best_action[5]
    ...
    try:
        for episode in range(int(benchmark.train_episodes)):
            ...
            while not (terminated or truncated):
                ...
                if episode > 0:
                    action = np.clip(action + rng.normal(0.0, 0.08, size=agent.action_dim), -1.0, 1.0).astype(np.float32)
