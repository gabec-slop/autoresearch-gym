...
except ModuleNotFoundError as exc:  # pragma: no cover - optional task extra.
    raise ModuleNotFoundError("PandaPickAndPlace requires the `panda` extra.") from exc
...
EXP_NAME = "panda_pick_and_place_dense_v0_cleanrl_sac_her_seed"
ALGORITHM = "sac"
CONTROL_TYPE = None
...
def get_candidate() -> str:
    return (
        "CleanRL-style SAC+HER seed for the Panda pick-and-place benchmark. Uses the "
        "same flattened goal observation and SAC update as the default seed, "
        "but relabels a configurable fraction of replay samples with achieved "
        "goals and recomputed task rewards. Warmup, relabel fraction, batch "
        "size, and update cadence are intentionally editable in this file."
    )
...
class RewardRecipeWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env[Any, Any], recipe: str | None = None) -> None:
        ...
        if self.recipe != "task_dense":
            raise ValueError(f"Unknown PandaPickAndPlace reward recipe: {self.recipe}")
        assert isinstance(env.observation_space, spaces.Dict)
        self.raw_observation_space = env.observation_space
        self.observation_dim = int(np.prod(env.observation_space.spaces["observation"].shape))
        self.goal_dim = int(np.prod(env.observation_space.spaces["achieved_goal"].shape))
    ...
    def compute_reward(self, achieved_goal: np.ndarray, desired_goal: np.ndarray) -> np.ndarray:
        reward = self.env.unwrapped.compute_reward(achieved_goal, desired_goal, {})
...
class HerReplayBuffer:
    ...
    def sample(self, batch_size: int, device: torch.device) -> tuple[torch.Tensor, ...]:
        ...
        if relabel_mask.any():
            ...
            achieved_start = self.observation_dim
            desired_start = self.observation_dim + self.goal_dim
            relabel_goals = self.next_observations[future_inds, achieved_start:desired_start]
            observations[relabel_mask, desired_start:desired_start + self.goal_dim] = relabel_goals
            next_observations[relabel_mask, desired_start:desired_start + self.goal_dim] = relabel_goals
            next_achieved = next_observations[relabel_mask, achieved_start:desired_start]
            rewards[relabel_mask, 0] = self.reward_fn(next_achieved, relabel_goals)
