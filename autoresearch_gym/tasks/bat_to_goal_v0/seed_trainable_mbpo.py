...
REWARD_RECIPE = "goal_delta_heavy_mbpo"
RECIPE = {
    ...
    "world_model": {
        ...
        "rollout_horizon": 1,
        "rollout_starts": 512,
        "model_replay_size": 120000,
        "batch_fraction": 0.25,
        "uncertainty_threshold": 1.0,
        "priority_fraction": 0.75,
        "priority_signal": "contacted_ball_or_near_ball",
    },
}
DIAGNOSTIC_SERIES = {
    "title": "Bat-to-goal MBPO diagnostics",
    "series": [
        {"key": "contacted_ball_rate", "label": "Contact", "color": "#2563eb", "source": "info_metrics", "chart": "normalized_line", "group": "interaction"},
        {"key": "ball_goal_distance", "label": "Ball Goal", "color": "#dc2626", "source": "info_metrics", "chart": "normalized_line", "group": "interaction"},
        {"key": "ball_velocity_toward_goal", "label": "Goal Vel", "color": "#16a34a", "source": "info_metrics", "chart": "normalized_line", "group": "interaction"},
        {"key": "model_delta_rmse", "label": "Model Delta", "color": "#7c3aed", "source": "info_metrics", "chart": "normalized_line", "group": "world_model"},
        {"key": "model_reward_rmse", "label": "Model Reward", "color": "#f97316", "source": "info_metrics", "chart": "normalized_line", "group": "world_model"},
        {"key": "model_replay_size", "label": "Synthetic", "color": "#0891b2", "source": "info_metrics", "chart": "normalized_line", "group": "world_model"},
    ],
}
...
def get_candidate() -> dict[str, Any]:
    return {
        "description": (
            "CleanRL-style SAC+MBPO bat-to-goal seed. It trains a state-action "
            "ensemble dynamics model online, starts short horizon-1 synthetic "
            "rollouts from contact/near-ball replay states, and mixes synthetic "
            "transitions into SAC minibatches under the fixed benchmark eval."
        ),
        "recipe": RECIPE,
...
class RewardRecipeWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env[np.ndarray, np.ndarray], recipe: str | None = None) -> None:
        super().__init__(env)
        self.recipe = recipe or REWARD_RECIPE
        if self.recipe not in {"task_dense", "goal_delta_heavy_mbpo"}:
            raise ValueError(f"Unknown bat-to-goal MBPO reward recipe: {self.recipe}")
    ...
    def step(self, action: np.ndarray):
        ...
        if self.recipe == "task_dense":
            ...
            velocity = float(info.get("ball_velocity_toward_goal", 0.0))
            goal_delta = self._prev_ball_goal_distance - ball_goal_distance
            shaped = -0.01
            if first_contact:
                shaped += 0.35
            if contacted:
                shaped += 0.25 * goal_delta
                shaped += 0.10 * np.clip(velocity, -3.0, 3.0)
            if success:
                shaped += 25.0
        ...
        info["training_reward"] = float(shaped)
        return obs, float(shaped), terminated, truncated, info


def _priority(info: dict[str, Any], next_obs: np.ndarray) -> float:
    del next_obs
    if bool(info.get("contacted_ball", False)) or bool(info.get("first_contact", False)):
        return 1.0
    ball_goal_distance = float(info.get("ball_goal_distance", 1.0))
    velocity = float(info.get("ball_velocity_toward_goal", 0.0))
    return 1.0 if ball_goal_distance < 0.35 or velocity > 0.0 else 0.0


def _record_extra(info: dict[str, Any]) -> dict[str, Any]:
    return {
        "contacted_ball_rate": float(bool(info.get("contacted_ball", False))),
        "ball_goal_distance": float(info.get("ball_goal_distance", 0.0)),
        "ball_velocity_toward_goal": float(info.get("ball_velocity_toward_goal", 0.0)),
    }
