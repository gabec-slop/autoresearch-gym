...
EXP_NAME = "inverted_pendulum_v5_cleanrl_sac_seed"
ALGORITHM = "sac"
CONTROL_TYPE = None
...
def get_candidate() -> str:
    return (
        "CleanRL-style single-file SAC seed for MuJoCo InvertedPendulum-v5. "
        "The candidate is the code in this file: actor/critic definitions, replay, "
        "losses, update cadence, logging, and reward handling are intentionally "
        "expressed as mutable Python rather than as a parameter object. "
        "Evaluation is owned by the runner and should not be modified here."
    )
...
class RewardRecipeWrapper(gym.Wrapper):
    def __init__(self, env: gym.Env[Any, Any], recipe: str | None = None) -> None:
        ...
        if self.recipe != "task_reward":
            raise ValueError(f"Unknown InvertedPendulum reward recipe: {self.recipe}")
