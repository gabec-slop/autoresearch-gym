...
class AutoresearchMujocoPandaPickAndPlaceEnv(gym.Env[dict[str, np.ndarray], np.ndarray]):
    ...
    def __init__(
        ...
        **_: Any,
        ...
        if self.robot_qpos_adrs.size:
            ...
            if self.robot_qpos_adrs.size > 7:
                self.home_qpos[self.robot_qpos_adrs[7:]] = 0.035
        ...
        goal_space = spaces.Box(
            low=np.asarray([-XY_RANGE, -XY_RANGE, CUBE_Z], dtype=np.float32),
            high=np.asarray([XY_RANGE, XY_RANGE, CUBE_Z + GOAL_Z_RANGE], dtype=np.float32),
        )
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        if initial_goal_distance < SUCCESS_THRESHOLD:
            raise ValueError(
        ...
        self.data.ctrl[:] = _denormalize_action(np.zeros(self.model.nu, dtype=np.float32), self.ctrl_low, self.ctrl_high)
    ...
    def compute_reward(self, achieved_goal: np.ndarray, desired_goal: np.ndarray, info: dict[str, Any] | None = None):
        ...
        success = _pick_place_success(distance, lifted_ever)
        if self.reward_type == "sparse":
            return -(~success).astype(np.float32)
        if info is not None and "lifted_ever" in info:
    ...
    def _info(self, obs: dict[str, np.ndarray]) -> dict[str, Any]:
        ...
        placed = bool(_pick_place_success(cube_to_goal, lifted_ever))
        gripper_closed = bool(_finger_width(self.data, self.robot_qpos_adrs) < 0.035)
        return {
...
class MujocoWarpPandaPickAndPlaceVectorEnv:
    ...
    def _sample_cube_positions(self, count: int) -> np.ndarray:
        return np.column_stack(
            [
                self.rng.uniform(-XY_RANGE, XY_RANGE, size=count),
                self.rng.uniform(-XY_RANGE, XY_RANGE, size=count),
                np.full(count, CUBE_Z),
            ]
        ).astype(np.float32)
    ...
    def reset(self, seed: int | None = None) -> np.ndarray:
        ...
        self.warp_data.ctrl.assign(np.repeat(_denormalize_action(np.zeros(self.model.nu), self.ctrl_low, self.ctrl_high)[None, :], self.num_envs, axis=0).astype(np.float32))
    ...
    def step(self, actions: np.ndarray):
        ...
        cube_at_goal = cube_to_goal < SUCCESS_THRESHOLD
        placed = cube_at_goal & self.lifted_ever
        ...
        infos = {
            ...
            "cube_lift_height": cube_lift,
            "near_cube": ee_to_cube < 0.055,
            "gripper_closed_near_cube": ee_to_cube < 0.055,
            "lifted": lifted,
...
def _finger_width(data: Any, robot_qpos_adrs: np.ndarray) -> float:
    if robot_qpos_adrs.size < 2:
        return 1.0
    return float(np.sum(np.maximum(0.0, np.asarray(data.qpos[robot_qpos_adrs[-2:]], dtype=np.float32))))
